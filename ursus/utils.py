...
def make_picture_element(context: Context, entry_uri: EntryURI, img_attrs={}, sizes=None) -> ElementTree.Element:
    ...
    # 1. Context'te entry veya transforms yoksa atla ve placeholder koy
    try:
        transforms = context['entries'][entry_uri]['transforms']
    except KeyError as e:
        print(f"[UYARI] Görsel veya transform bulunamadı: {entry_uri} -- placeholder ile devam ediliyor.")
        picture = ElementTree.Element('picture')
        img = ElementTree.SubElement(picture, 'img', attrib=img_attrs)
        img.attrib['src'] = "https://placehold.co/600x400?text=Image+not+found"
        img.attrib['alt'] = "Eksik görsel"
    ...
    if default_src is not None:
        ...
        print(f"[UYARI] default_src bulunamadı: {entry_uri} -- placeholder ile devam ediliyor.")
        img.attrib['src'] = "https://placehold.co/600x400?text=Image+not+found"
    img.attrib['loading'] = 'lazy'
